...
4. **Duplicate Row Upsert Logic**: Re-wrote the duplicate checking logic in `POST /api/locations/import-csv`. The route now geocodes the address *before* comparing against the database to prevent raw string mismatches. It then compares the uploaded row against the database record: if it is an exact duplicate (all fields match including `notes` and `status`), it categorizes it as ignored. If the fields differ, it executes an `UPDATE` (upsert) to cleanly overwrite the existing location with the new CSV data rather than throwing a `UNIQUE` constraint violation.
